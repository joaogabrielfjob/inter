...
function GoalList({ goals, away = false }: { goals: ResultCardProps['goalSummary']['goals']; away?: boolean }) {
  return <ul className='mt-4 space-y-1 text-sm'>{goals.map((goal, index) => <li key={`${goal.scorer}-${goal.minute}-${index}`} className='whitespace-nowrap'>{away ? `${formatAwayMinute(goal.minute)} — ${goal.marker ? `(${goal.marker}) ` : ''}${goal.scorer}` : `${goal.scorer}${goal.marker ? ` (${goal.marker})` : ''} — ${formatMinute(goal.minute)}`}</li>)}</ul>;
}
