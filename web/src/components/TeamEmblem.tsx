...
export function TeamEmblem({ src, name, className = 'h-12' }: { src?: string; name: string; className?: string }) {
  const [failed, setFailed] = useState(false);
  const resolvedSrc = !src || failed ? fallback : new URL(src, import.meta.env.VITE_API_URL).toString();

  return <img src={resolvedSrc} alt={`${name} emblem`} className={className} onError={() => setFailed(true)} />;
}
